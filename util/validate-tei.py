...
def check_footnotes(doc, errors):
    ...
    for note in markup:
        if len(note) == 0:
            errors.append('Empty footnote ({0})'.format(note.attrib['{{{0}}}id'.format(ns['xml'])][1:] if '{{{0}}}id'.format(ns['xml']) in note.attrib else '?'))
...
def check_editor_transition(doc, errors):
    ...
    for ref in references:
        if 'type' not in ref.attrib and 'target' not in ref.attrib:
            ...
            if 'type' in ref.attrib and ref.attrib['type'] not in ('esv', 'footnote', 'external', 'global'):
